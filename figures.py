...
def make_reviews_counts_fig(books_df):
    ...
    # horizontal reference lines as scatter (so legend shows)
...
def make_top_authors_fig(books_df, n):
    ...
    for _, row in top_authors.iterrows():
        fig.add_annotation(x=row["author"], y=row["book_count"] + 0.5,
                           text=str(int(row["book_count"])), showarrow=False,
                           font=dict(size=10, color="steelblue"), yanchor="bottom")
    fig.update_layout(
...
def make_top_genres_fig(books_df, n):
    ...
    for _, row in genre_stats.iterrows():
        fig.add_annotation(x=row["genre"], y=row["book_count"] + 0.5,
                           text=str(int(row["book_count"])), showarrow=False,
                           font=dict(size=10, color="steelblue"), yanchor="bottom")
    fig.update_layout(
